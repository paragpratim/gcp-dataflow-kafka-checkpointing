...
public class ExtractHandledWriteOffsetsFn extends SimpleFunction<TableRow, KV<String, Long>> {
    ...
    public KV<String, Long> apply(TableRow row) {
        ...
        Object topic = row.get(WriteRawMessageTransform.META_KAFKA_TOPIC);
        Object partition = row.get(WriteRawMessageTransform.META_KAFKA_PARTITION);
        Object offset = row.get(WriteRawMessageTransform.META_KAFKA_OFFSET);
